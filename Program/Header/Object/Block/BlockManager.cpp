...
IBlock *const ModelHandleDrawList::push_back(IBlock &&block)
{
	blocks_.push_back(std::move(block));
	return &blocks_.back();
}
...
void ModelHandleListManager::Init(uint32_t maxCount)
{
	blocks_.CreateBuffer(maxCount);
...
void SkinModelHandleListManager::Init(uint32_t maxCount)
{
	blocks_.CreateBuffer(maxCount);
